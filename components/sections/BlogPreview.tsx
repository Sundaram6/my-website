        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="flex justify-between items-end mb-12 flex-wrap gap-4">
            <div>
              ...
              <p className="text-slate-400 text-[1.05rem] max-w-lg">Musings on AI, engineering, and building products.</p>
            </div>
          ...
          {[...posts].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 3).map((post, i) => (
            ...
            >
              <Link href={`/blog/${post.slug}`} className="p-6 flex flex-col h-full">
                <div className="flex gap-2 flex-wrap mb-4 text-[0.75rem] font-medium text-slate-500">
                  <span>{post.date}</span>
                  <span>•</span>
                ...
                <p className="text-slate-400 text-[0.875rem] leading-relaxed mb-6 flex-grow">{post.excerpt}</p>
